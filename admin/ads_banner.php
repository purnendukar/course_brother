        ...
        <script>
                                            ...
                                            $.ajax({
                                                ...
                                                complete: function (data) {
                                                    if(data.responseText==1){
                                                                                                ...
                                                                                                window.location.href="./ads_banner";
                                                }
                                            });
                                }
                            }
						...
						function add_it(){
                            if(confirm("Want to Update?")){if(confirm("Going to update")){
                                ...
                                if(link.value=="" || document.getElementById("img_choose").value==""){
                        ...
                        function send_data(id,link){
                            
                        }
						function change_(a){
                            var link=document.getElementById("link_"+a);
                            var img_c=document.getElementById("img_choose"+a);
                            var img_d=document.getElementById("img_display"+a);
                            if(link.disabled){
                                link.disabled=false;
                                img_c.style.display="";
                                img_d.style.display="none";
                                                ...
                                                if(confirm("Want to Update id "+a+" data?")){if(confirm("Going to Update id "+a+" data")){
                                                    ...
                                                    formData.append('image',$('#img_choose'+a)[0].files[0]);
                                                    formData.append('link',link.value);
                                                    $.ajax({
                                                        type: "POST",
                                                        cache: false,
						...
						function cancel_(a){
							...
							if(link.disabled==false){
								var formData= new FormData();
								formData.append('query','select * from sponsor_course where id='+a);
								$.ajax({
									url: "./get_info.php",
									type: 'POST',
									cache: false,
											...
											<table id="dynamic-table" class="table table-striped table-bordered table-hover">
												<div style="background-color: #EFF3F8;padding:15px 15px 5px 15px;">
													...
													<div id="add_one" class="form-group"  style="display:none;width:100%;text-align:center; padding:30px;">			
														<input type="text" class="form-control" id="link"  style="margin:5px;" />
												...
												<tbody>
												<?php
                                                    $sql="SELECT * FROM sponsor_course";
												...
												"<tr id='row_".$row['id']."'>
                                                        ...
                                                        <td>
                                                            <input class='lbl' id='link_".$row['id']."' style='border:none;padding:5px;width:100%;' disabled value='".$row['link']."'/>
                                                        </td>
														<td class='hidden- center'>
															<span class='label label-sm label-warning' style='height:auto;font-size:13px;' id='link".$row['id']."' >".$row['update_by']." <br/>".implode('-',array_reverse(explode('-',explode(' ',$row['updated'])[0])))."<br>".explode(' ',$row['updated'])[1]."</span>
														</td>
